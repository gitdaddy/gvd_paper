...
%  \LinesNumbered
  ...
  \BlankLine
  \tcp{Initialization}
  $beachline$ := the line equidistant between the event and the event\;
  $closeEventPoints$ := the set of all close \;
  \While{events > 0 and exists events in sweepline path}{
  	let $e$ := the next event\;
    \If{$e$ is close event}{
    	\If{$e$ is live}{
    	  update arch node points\;
    	  remove $e.archNode$ from the $beachline$\;
% 		  \ForEach{$ev$ in events}{
% 		  	\If{$ev.y$ less than $e.y$ - \varepsilon}{
% 		  		add $ev$ to events\;
% 		  		\If{$ev$ is close event}{
